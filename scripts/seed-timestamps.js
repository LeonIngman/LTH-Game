...
async function seedTimestamps() {
  ...
  try {
    ...
    const existingTimestamps = await pool.query(
      'SELECT COUNT(*) as count FROM "TimeStamp"'
    );

    if (Number.parseInt(existingTimestamps.rows[0].count) > 0) {
      console.log(
        `Found ${existingTimestamps.rows[0].count} existing timestamps, skipping seed.`
      );
      return;
    }
    ...
    for (let levelId = 0; levelId <= 3; levelId++) {
      ...
      for (let day = 1; day <= 30; day++) {
        ...
        const fluctuationFinished =
          Math.sin(day / 3) * 5 + (Math.random() * 4 - 2);

        const rawMaterialAPrice = Number.parseFloat(
          (basePriceA + fluctuationA).toFixed(2)
        );
        const rawMaterialBPrice = Number.parseFloat(
          (basePriceB + fluctuationB).toFixed(2)
        );
        const finishedGoodPrice = Number.parseFloat(
          (baseFinishedPrice + fluctuationFinished).toFixed(2)
        );
