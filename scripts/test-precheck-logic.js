...
function simulateCheckSufficientFunds(gameState, action, levelConfig) {
  ...
  function calculateTotalCost() {
    ...
    const holdingCost = Object.values(gameState.inventory || {}).reduce((sum, qty) => sum + qty, 0) * 0.1
...
const testScenarios = [
  {
    ...
    action: {
      supplierOrders: [
        { supplierId: 1, pattyPurchase: 100, cheesePurchase: 80, bunPurchase: 120, potatoPurchase: 150 }
      ],
      production: 50,
      customerOrders: []
    },
  ...
  },
  {
    ...
    action: {
      supplierOrders: [
        { supplierId: 1, pattyPurchase: 20, cheesePurchase: 15, bunPurchase: 25, potatoPurchase: 30 }
      ],
      production: 10,
      customerOrders: []
    },
  ...
  },
  {
    ...
    action: {
      supplierOrders: [
        { supplierId: 1, pattyPurchase: 50, cheesePurchase: 40, bunPurchase: 60, potatoPurchase: 70 }
      ],
      production: 15,
      customerOrders: []
    },
...
testScenarios.forEach((scenario, index) => {
  ...
  const result = simulateCheckSufficientFunds(scenario.gameState, scenario.action, {})
  ...
  if (!testPassed) {
    console.log(`   - Expected: ${scenario.expectedSufficient}, Got: ${result.sufficient}`)
  }
})
