// Run this script with: node scripts/test-db-connection.mjs
...
async function testConnection() {
  ...
  try {
    ...
    console.log(
      "SSL Mode:",
      url.searchParams.get("sslmode") || "not specified"
    );
  } catch (parseError) {
  ...
  try {
    ...
    if (error.message.includes("Failed to fetch")) {
      ...
      console.log(
        "3. Neon database is not accessible from your current network"
      );
      console.log("4. The database URL is incorrect");

      console.log("\nTroubleshooting steps:");
      console.log(
        "1. Check if you can access the Neon console in your browser"
      );
